...
${tks.filter(t=>!t.done).map(t=>`・${t.title}（優先度:${t.prio}）${t.due?'期限:'+t.due:''}`).join('\n')||'なし'}
  ...
  if(page==="autoedit"){
    ...
    const runEdit=async()=>{
      ...
      }catch(e){setAeResult({ok:false,msg:"通信エラーが発生しました"});setAeHistory(prev=>prev.map((h,i)=>i===0?{...h,status:"❌ エラー"}:h));}
      setAeLoading(false);
    };
    if(!aeUnlocked)return(
      <div style={{fontFamily:"'Hiragino Sans','Yu Gothic',sans-serif",background:"#F0F4F8",minHeight:"100vh",...pp}}>
        {isPC&&(cust.showSidebar!==false)&&<PCSidebar/>}{isPC&&(cust.showRightPanel!==false)&&<PCRightPanel/>}{(cust.showLauncher!==false)&&<FloatLauncher/>}
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"70vh",padding:24}}>
          <div style={{background:"#fff",borderRadius:20,padding:32,width:"100%",maxWidth:320,textAlign:"center",boxShadow:"0 4px 20px rgba(0,0,0,0.1)"}}>
            ...
            <input type="password" value={aePin} onChange={e=>setAePin(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){if(aePin===AE_PIN){setAeUnlocked(true);setAePin("");}else{alert("パスコードが違います");setAePin("");}}}
            ...
            <button onClick={()=>{if(aePin===AE_PIN){setAeUnlocked(true);setAePin("");}else{alert("パスコードが違います");setAePin("");}}} style={{width:"100%",padding:"13px 0",background:"#6D28D9",color:"#fff",border:"none",borderRadius:10,fontWeight:800,fontSize:15,cursor:"pointer"}}>ロック解除</button>
          </div>
        </div>
      </div>
    );
